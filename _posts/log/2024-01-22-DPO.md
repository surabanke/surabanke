...
GPT-3이라고 불리는 Instruct GPT의 RLHF가 잘 알려져있다.
여기서는 3가지 단계로 GPT를 튜닝한다.

첫번째로 휴먼 라벨러가 직접 선호도를 체크한 데이터 셋으로 학습 한다.
...
<img src = "/surabanke/assets/images/BT_Model.png" width = "300">

위 식은 Bradley-Terry model이라고 하며 preference modeling할때 많이 사용하는 수식이라고한다.
...
DPO는 PPO-based RLHF의 두 절차를 하나로 결합한다.
RM을 없애고 binary cross entropy objective를 이용해 인간의 선호도에 기반한 policy를 직접 생성한다.

<img src = "/surabanke/assets/images/objectiveRLHF2.png" width = "300">

처음에는 위의 RLHF objective function을 그대로 가져온다.
수식은 아래와 같이 변형해준다.

<img src = "/surabanke/assets/images/change_shape.jpeg" width = "400">


마지막 형광줄이 그어져있는 식은 결국 아래식과 같다. 업데이트 이전의 policy와 이후 policy 차이에 따라 KL패널티를 주는 것이 RLHF와 같다.

<img src = "/surabanke/assets/images/DPO_1.png" width = "300">

즉 업데이트 이전의 policy가 최적이 될때가 목표가되기때문에 형광줄 식에서 분모값이 optimal solution이된다.

<img src = "/surabanke/assets/images/DPO_2.png" width = "300">

위 식은 아래와 같이 r(x,y)형태로 나타낼수있으며 여러 변형을 거쳐 DPO의 loss식으로 거듭난다.

<img src = "/surabanke/assets/images/DPO_3.png" width = "400">
...
이를 사용하지 않고 likelihood만 가지고 학습한다면 성능이 급감소한다고한다.)
